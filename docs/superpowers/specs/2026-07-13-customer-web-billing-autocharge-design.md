...
- `pnpm gen:types` 재실행: `../carry-platform/docs/api/openapi-v2.json` → `packages/types/src/generated/v2.ts`. 새 스키마(`BillingKeyResponse`, `/api/v2/billing-keys` 경로, 6-값 order status, `OVERDUE`, `OrderResponse`에서 `totalAmount` 제거)가 자동 반영된다.
- 앱 측 anti-corruption 매퍼(기존 `to*` 함수 옆에 배치):
  - `toBillingKey(res)` — 생성 `BillingKeyResponse` → 앱 `BillingKey{cardCompany, cardLast4, registeredAt}`.
  - `toDeliveryProgress(orderStatus)` — 주문 6상태 → 친화 단계(§4.4).
  - `toPaymentStatus(invoice, payment)` — invoice/payment 상태 → 결제 배지 모델(§4.4).
...
2. `postOrder(...)` 호출. 409 응답 분기:
   - `BILLING_KEY_REQUIRED` — 조회-제출 사이 레이스 폴백: 등록 시트 오픈.
   - `OVERDUE_INVOICE_EXISTS` — 연체 해소 화면으로 라우팅(§4.6).
   - 그 외 에러 — 기존 토스트.
3. 성공 → `reset()` → `/status/{id}`.

에러 코드 판별은 `@carry/api`가 언랩하는 `ApiResponse.error.code` 기준. 백엔드 코드 문자열(`BILLING_KEY_REQUIRED` 등)을 앱 상수로 매핑.
...
- **결제 배지** (`toPaymentStatus`): 인보이스/결제 조회(`GET /api/v2/payments/{orderId}/invoice`, `/payment`) →
  ...
  | Payment PENDING/FAILED, Invoice ISSUED | 결제 처리중 |
  | Invoice OVERDUE 또는 Payment FAILED(장기) | 결제 실패/연체 |
  - 현재 `{}원` 스텁을 실 `invoice.totalAmount`로 교체.
- **실패/연체 배너**: 결제 실패·연체 시 "결제 수단에 문제가 있어요 · 카드 확인 필요" + CTA → 카드 변경(§4.2)/연체 해소(§4.6).
- 인보이스 조회는 주문 상세와 병렬 `useQuery`; 수거 전(인보이스 404)은 결제 지표 숨김.
...
진입: 주문 제출 409 OVERDUE 또는 상태 화면 연체 배너.
화면(신규 `features/billing/ui/OverdueResolution.tsx` 또는 라우트): "미납 결제가 있어 새 주문을 만들 수 없어요. 카드를 다시 등록하면 자동으로 재결제됩니다." → 카드 재등록 시트 → 등록 성공 후 안내("재결제가 진행 중입니다") → `useMyBillingKey`·연체 상태 재조회(폴링 또는 pull-to-refresh)로 해소 확인. 백엔드 `ChargeRetrySweeper`가 실제 재과금을 수행하므로 프론트는 재등록 + 상태 확인만 담당. 해소되면 주문 플로우 재개.
...
| getMyBillingKey 404 | 카드 없음(등록 유도) |
