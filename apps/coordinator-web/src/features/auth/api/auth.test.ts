...
describe('auth API (dev-login / refresh)', () => {
  it('devLogin은 역할 토큰쌍을 반환한다', async () => {
    const tokens = await devLogin('COORDINATOR');
    expect(tokens).toEqual({ accessToken: 'mock-access', refreshToken: 'mock-refresh' });
  });

  it('devLogin은 기본 역할로 COORDINATOR를 본문에 담아 보낸다', async () => {
    ...
    await devLogin();
    expect(captured).toEqual({ role: 'COORDINATOR' });
  });

  it('devLogin은 ADMIN 역할도 담아 보낸다', async () => {
    let captured: unknown;
    server.use(
      http.post('*/api/v2/auth/dev-login', async ({ request }) => {
        captured = await request.json();
