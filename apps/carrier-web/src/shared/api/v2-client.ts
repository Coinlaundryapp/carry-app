...
/**
 * carry-platform v2 백엔드용 클라이언트(carrier-web).
 *
 * **토큰 소유권**: carrier-web은 NextAuth가 없다(dev-login 전용 내부툴). 앱이 토큰을 직접
 * 소유하고 `@carry/api`의 refresh coordinator를 **활성**한다 — 401이면 single-flight로
 * `/v2/auth/refresh` 회전 후 1회 재시도. 동시 회전은 백엔드(#82) 재사용 감지로 세션 폐기를
 * 부르므로 single-flight가 자폭을 막는다. (customer-web은 반대로 NextAuth가 회전을 소유해
 * coordinator를 끈다.)
 */
