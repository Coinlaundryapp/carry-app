...
/**
 * coordinator 인증 API — dev-login(역할 토큰 발급)과 refresh(토큰 회전).
 *
...
/** coordinator-web이 다루는 역할 — 운영(COORDINATOR)과 관리자(ADMIN). */
...
export async function devLogin(role: CoordinatorRole = 'COORDINATOR'): Promise<TokenResponse> {
