  ...
  if (item.id === 'service-center') {
    return (
      <a href={item.path} className="flex w-full items-center justify-between">
        <span className="font-semibold text-label-neutral font-body-1-normal">{item.title}</span>
        <span className="flex items-center">
          {item.rightText && (
            <span className="mr-3 rounded-md bg-cyan-50 px-2.5 py-1 font-semibold text-primary-normal font-label-1-normal">
              {item.rightText}
            </span>
          )}
  ...
  return (
    <div>
      <div className="mb-5 flex items-center gap-1">
        {menu.icon}
        <h2 className="font-semibold text-label-normal font-headline-1">{menu.title}</h2>
      </div>
